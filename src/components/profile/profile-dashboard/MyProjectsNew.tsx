...
const MyProjects = ({ projectData }: { projectData: IProject }) => {
  ...
  const unlockDate = useMemo(() => {
    ...
    if (projectData?.seasonNumber === 1) {
      ...
      return allVestingData.find(
        period =>
          period.type === 'team' &&
          period.season === (projectData?.seasonNumber || 2)
      )?.cliff;
    }
  ...
  useEffect(() => {
    ...
    let activeRound: IEarlyAccessRound | IQfRound = {} as
      | IEarlyAccessRound
      | IQfRound;
    const pastRounds: (IEarlyAccessRound | IQfRound)[] = [];
    let roundType = 'ea';
    ...
    allRoundData.forEach(round => {
      ...
      const isActive = isMiddleOfThePeriod(startDate, endDate);
      if (
  ...
  return (
    <div className=''>
        ...
        <div className='col-span-12 md:col-span-6 p-4 h-64 relative'>
          ...
          <div className='absolute top-0 px-8 left-0 w-full h-full flex flex-col justify-center'>
            ...
            <div className='flex gap-2 mt-4'>
              {filteredRoundData.activeRound.endDate >
      ...
      {filteredRoundData.activeRound &&
        (filteredRoundData.activeRound as any).startDate && (
            ...
            <div className='py-5 px-4 md:px-12 bg-white/[5%] rounded-3xl'>
              ...
              <div className='block md:hidden'>
                ...
                <div className='grid grid-cols-2 gap-4'>
                  ...
                  <div className='space-y-0.1'>
                    <div className='text-white text-center text-xl font-bold'>
                      {tokenDetails?.issuance_supply
                        ? (
                            (Number(mintedTokenAmounts) /
                    ...
                    <div className='text-white text-center text-xl font-bold'>
                      {unlockDate ? (
                        <span className='text-white'>
                          {unlockDate.toLocaleString('default', {
                            month: 'short',
                            day: 'numeric',
                          })}
                        </span>
              ...
              <div className='block md:hidden'>
                ...
                <div className='grid grid-cols-2 gap-4'>
                  <div className='space-y-0.1'>
                    <div className='text-white text-center text-xl font-bold'>
                      {tokenDetails?.issuance_supply
                        ? formatNumber(Number(tokenDetails.issuance_supply))
                        : '0'}
                    </div>
                    <div className='text-white/30 text-center font-medium text-[13px] leading-normal'>
                      Total Supply
