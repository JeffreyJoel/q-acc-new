...
export const DonorProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  >({});
  const [totalUserContributions, setTotalUserContributions] =
    useState<number>(0);
  const [totalUserContributionsUsd, setTotalUserContributionsUsd] =
    useState<number>(0);
  const [contributedProjectsCount, setContributedProjectsCount] =
    useState<number>(0);
  const [contributedRoundsCount, setContributedRoundsCount] =
    useState<number>(0);
        ...
        try {
          ...
          if (donationsByProjectId?.donations) {
            ...
            donorData[projectId] = {
              ...
              userProjectContributionSumUsd:
                calculateTotalContributionsUsd(userDonation),
            };
          }
        } catch (err) {
