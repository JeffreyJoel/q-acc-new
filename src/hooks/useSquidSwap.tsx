...
export const useSquidSwap = () => {
  ...
  const getQuote = useCallback(async (params: SwapParams): Promise<SwapQuote | null> => {
    if (!squid || !userAddress) {
      setStatus(prev => ({ ...prev, error: "Squid SDK not initialized or user not connected" }));
      return null;
    }
    ...
    try {
      ...
      const quote: SwapQuote = {
        fromAmount: formatUnits(BigInt(route.estimate.fromAmount), params.fromDecimals),
        toAmount: formatUnits(BigInt(route.estimate.toAmount), params.toDecimals),
      ...
      if (error instanceof Error) {
        if (error.message.includes("Token is not supported")) {
          errorMessage = "This token is not supported for swapping. Please try a different token.";
  ...
  const approveTokenIfNeeded = useCallback(async (
  ...
  const executeSwap = useCallback(async (params: SwapParams): Promise<string | null> => {
    ...
    try {
      ...
      // Get target address for approval
      let targetAddress: string;
      if ('target' in route.transactionRequest) {
        targetAddress = route.transactionRequest.target;
      } else {
        throw new Error("Cannot determine target address from transaction request");
      }
  ...
  return {
    ...
    isLoading: status.isInitializing || status.isQuoting || status.isSwapping || status.isApproving,
    error: status.error,
